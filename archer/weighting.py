...
def lf_observable_fraction(mags, wght, d_kpc, filters,mask=True,
    ...
    for i, m in enumerate(mu):
        mag = mags[:, filters.index(selection_band)] + m
        valid = (mag > bright) & (mag < faint) & mask
        if wisecut:
            w1 = mags[:, filters.index("WISE_W1")] + m
            valid = valid & (w1 < 15.5)
...
def mag_weight(dist, feh, iso, masker=None,
    ...
    valid = (mag > bright) & (mag < faint) & mask
    if wisecut:
        w1 = mags[:, iso.filters.tolist().index("WISE_W1")] + mu
        valid = valid & (w1 < 15.5)
...
def logg_masker(params, mags, filters):
    """Default: apply mask based on logg, remove mgiants.
    """
    sel = params["logg"] < 3.5
    sel = sel & (~mgiant_masker(params, mags, filters))
    return sel


def mgiant_masker(params, mags, filters):
    """Choose mgiants basd on Conroy+ 2019.  Does not include apparent mag
...
def bhb_masker(params, mags, filters):
    ...
    #ps_g_r = mags[:, filters.index("PS_g")] - mags[:, filters.index("PS_r")]
    #g_r = mags[:, filters.index("SDSS_g")] - mags[:, filters.index("SDSS_r")]
    #u_g = mags[:, filters.index("SDSS_u")] - mags[:, filters.index("SDSS_g")]
...
def compute_total_weight(rcat_row, iso, wcat, snr_limit=3.0, use_ebv=False, 
    ...
    use_ebv : bool (optional, default=False)
        Adjust the weights to account for a foreground screen when computing the
        observability of isochrone points.
...
if __name__ == "__main__":
    ...
    bmask = bhb_masker(params, mags, iso.filters.tolist())

    d_arr = np.linspace(2, 100, 1000)
    frac_g = lf_observable_fraction(mags, weights, d_arr, filters.tolist(), mask=gmask)
    frac_m = lf_observable_fraction(mags, weights, d_arr, filters.tolist(), mask=mmask,
                                    faint=17.5, bright=13.5, wisecut=True)
    frac_b = lf_observable_fraction(mags, weights, d_arr, filters.tolist(), mask=bmask,
                                    faint=17.5, bright=13.5)
    frac = lf_observable_fraction(mags, weights, d_arr, filters.tolist())
