...
def show_vfit(lam, mu, sig, nsig=1, ax=None, color="k", 
    ...
    if nsig > 0:
        ax.fill_between(lam[o], mu[o] - nsig * sig[o], mu[o] + nsig * sig[o],
                        alpha=0.3, color=color, **fill_kwargs)
    return ax
...
if __name__ == "__main__":
    ...
    # plot fits
    show_vfit(rcat_r["lambda"][tsel & gt], tmu[tsel & gt], tsig[tsel & gt], color=fcolor, ax=vtax)
    show_vfit(rcat_r["lambda"][tsel & gt], mock_tmu[tsel & gt], mock_tsig[tsel & gt],
              nsig=0, color="orange", ax=vtax, plot_kwargs=dict(linestyle="--"))
    show_vfit(rcat_r["lambda"][lsel], lmu[lsel], lsig[lsel], color=fcolor, ax=vlax)
    show_vfit(rcat_r["lambda"][lsel& gl], mock_lmu[lsel & gl], mock_lsig[lsel & gl],
              nsig=0, color="orange", ax=vlax, plot_kwargs=dict(linestyle="--"))
    
    # plot sigmas
    show_sfit(rcat_r["lambda"][tsel & gt], tsig[tsel & gt], samples=tsigs[:, tsel & gt],
              ax=stax, color=fcolor)
    show_sfit(rcat_r["lambda"][tsel & gt], mock_tsig[tsel & gt],
              ax=stax, color="orange", plot_kwargs=dict(linestyle="--", label="fit to LM10"))    
    show_sfit(rcat_r["lambda"][lsel], lsig[lsel], samples=lsigs[:, lsel],
              ax=slax, color=fcolor)
    show_sfit(rcat_r["lambda"][lsel & gl], mock_lsig[lsel & gl],
              ax=slax, color="orange", plot_kwargs=dict(linestyle="--"))    
    ...
    slax.errorbar(360 - gibl["lam"], gibl["vsig1"], yerr=gibl["vsig1_err"],
                  label="G17 (Metal Poor)", alpha=galpha, color="lightblue")
    slax.errorbar(360 - gibl["lam"], gibl["vsig2"], yerr=gibl["vsig2_err"],
                  label="G17 (Metal Rich)", alpha=galpha, color="maroon")
    ...
    stax.legend(fontsize=8, loc="upper left")
